...
import type { SupplyChainOfferingItem } from '../data/supply-chain.data';
import { CapabilityCard } from '@shared';
import { sectionContainer } from '@shared/constants';
import { FadeUp } from '@shared';

interface SupplyChainOfferingsGridProps {
...
export function SupplyChainOfferingsGrid({
  offerings,
  title,
  subtitle,
  lang,
}: SupplyChainOfferingsGridProps) {
  const isRtl = lang === 'ar';
      ...
      <div className={sectionContainer}>
        ...
        <FadeUp delay={0} duration={700} distance={20} className="text-center max-w-3xl mx-auto mb-14 sm:mb-20">
          <span className="text-xs sm:text-sm font-bold uppercase tracking-widest text-persici-crimson block mb-3">
            {isRtl ? 'القدرات وحلول سلاسل الإمداد' : 'Core Capabilities'}
          </span>
