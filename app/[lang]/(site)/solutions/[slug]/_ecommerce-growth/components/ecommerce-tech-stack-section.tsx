...
import type { TechStackPod } from '../data/ecommerce-growth.data';
import { sectionContainer } from '@shared/constants';
import { FadeUp } from '@shared';

interface EcommerceTechStackSectionProps {
  pods: TechStackPod[];
  title: string;
  subtitle: string;
  lang: string;
...
export function EcommerceTechStackSection({
  pods,
  title,
  subtitle,
  lang,
}: EcommerceTechStackSectionProps) {
  const isRtl = lang === 'ar';
                  ...
                  <div className="flex flex-wrap gap-2 pt-4 border-t border-white/5 mt-auto">
                    {pod.technologies.map((tech, tIdx) => (
                      ...
                      >
                        ...
                        <span>{tech.name}</span>
                        {tech.badge && (
                          <span className="text-[10px] font-mono text-persici-crimson/90 bg-persici-crimson/15 px-1.5 py-0.2 rounded">
                            {tech.badge}
                          </span>
                        )}
                      </span>
