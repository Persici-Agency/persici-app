...
import type { TechStackPod } from '../data/crm-management.data';
...
import { TbBrandAppgallery, TbMessageChatbot, TbDatabaseExport, TbShieldCheck } from 'react-icons/tb';

interface CrmTechStackSectionProps {
  pods: TechStackPod[];
  title: string;
  subtitle: string;
  lang: string;
...
export function CrmTechStackSection({
  pods,
  title,
  subtitle,
  lang,
}: CrmTechStackSectionProps) {
  const isRtl = lang === 'ar';
      ...
      <div className={sectionContainer}>
        ...
        <FadeUp delay={0} duration={700} distance={20} className="text-center max-w-3xl mx-auto mb-16 sm:mb-20 relative z-10">
          <span className="text-xs sm:text-sm font-bold uppercase tracking-widest text-persici-crimson block mb-3">
            {isRtl ? 'المنظومة التقنية لإدارة علاقات العملاء' : 'Enterprise CRM Technology Stack'}
          </span>
