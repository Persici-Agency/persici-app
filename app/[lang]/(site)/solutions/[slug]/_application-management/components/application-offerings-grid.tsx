...
import type { ApplicationOfferingItem } from '../data/application-management.data';
import { SolutionsVectorDiagram } from '../../../_solutions/components/solutions-vector-diagram';
import { sectionContainer } from '@shared/constants';
import { FadeUp } from '@shared';

interface ApplicationOfferingsGridProps {
...
export function ApplicationOfferingsGrid({
  offerings,
  title,
  subtitle,
  lang,
}: ApplicationOfferingsGridProps) {
  const isRtl = lang === 'ar';
      ...
      <div className={sectionContainer}>
        ...
        <FadeUp delay={0} duration={700} distance={20} className="text-center max-w-3xl mx-auto mb-14 sm:mb-20">
          <span className="text-xs sm:text-sm font-bold uppercase tracking-widest text-persici-crimson block mb-3">
            {isRtl ? 'ركائز التطوير' : 'Core Capabilities'}
          </span>
          ...
          {offerings.map((item, idx) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div className="group relative flex flex-col justify-between rounded-2xl bg-[#F8F7F4] border border-black/[0.04] p-6 sm:p-7 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-lg hover:border-black/[0.08] w-full">
                    {/* Top: Tag + Custom Icon */}
                    <div>
                    ...
                    {/* Bottom: Narrative + Capability Pills */}
                    <div>
        ...
        {/* Minimal Icon-Only Pause/Resume Animation Toggle on the side (matching Solutions page) */}
        <div className="mt-6 flex justify-end max-w-7xl mx-auto px-2">
          <button
            type="button"
